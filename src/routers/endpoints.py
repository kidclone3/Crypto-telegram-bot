...
from src.schemas import LoginSchema, QRLink, StausCheck, Messages, MessageCreate
from src.services import session_service, message_service
...
@router.get("/messages")
async def messages(
    phone: str, uname: str, session: AsyncSession = Depends(get_async_session)
) -> Messages:
...
async def send_message(
    message_data: MessageCreate, session: AsyncSession = Depends(get_async_session)
):
