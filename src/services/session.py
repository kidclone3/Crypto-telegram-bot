...
from src.repos import session_repo
from src.schemas import LoginSchema, QRLink, StausCheck
...
class SessionService:
    async def generate_client(self, phone: str):
        return TelegramClient(
            str(SESSIONS_DIR / phone), settings.api_id, settings.api_hash
        )
    ...
    async def login(
        ...
        tg_session = await session_repo.get_one_by_phone(login_data.phone, session)
        if tg_session:
