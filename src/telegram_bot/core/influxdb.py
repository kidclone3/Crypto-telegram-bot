...
class InfluxDB:
    ...
    async def write_trades(self, exchange, trades: List[Dict]):
        ...
        self.write_api.write(bucket="trades", org="trade", record=points)
    ...
    async def write_stats(self, exchange, stats, symbol):
        ...
        self.write_api.write(bucket="trade_stats", org="trade", record=point)
    ...
    async def write_ob_and_trades(self, exchange, trades, orderbook):
        ...
        self.write_api.write(bucket="market_data", org="trade", record=points)
    ...
    async def write_candles(self, all_candles: Dict[str, Dict[str, pd.DataFrame]]):
        ...
        self.write_api.write(bucket="candles", org="trade", record=points)
                ...
                for timestamp, row in dataframe.iterrows():
                    ...
                    if len(points) >= batch_size:
                        self.write_api.write(
                            bucket="candles", org="trade", record=points
                        )
    ...
    async def write_order_book(self, exchange, orderbook):
        ...
        points.append(order_book_point)
        self.write_api.write(bucket="orderbook", org="trade", record=points)
    ...
    def get_influxdb_client(self, is_local):
        return InfluxDBClient(
            ...
            token=(
                os.getenv(
                    "INFLUXDB_TOKEN_LOCAL"
                )  # if is_local else os.getenv("INFLUXDB")
            ),
            org="trade",
        )
